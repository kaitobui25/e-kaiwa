...
Use 0-100 scores. List at most 4 clear problems. If no clear problem, return an empty problems array.'''
...
def analyze(key, model, wav, reference):
    body = {"contents":[{"role":"user","parts":[{"text":prompt(reference)},{"inlineData":{"mimeType":"audio/wav","data":base64.b64encode(wav.read_bytes()).decode("ascii")}}]}],
...
def main():
    ...
    for i, row in enumerate(rows, 1):
        ...
        total = 0.0
        for model in MODELS:
            obj, dt, error = analyze(key, model, wav, ref); total += dt
            if obj is not None:
        ...
        if result is None:
            print("  RESULT: failed\n"); continue
        ok_count += 1; total_times.append(total); wins[used] = wins.get(used,0)+1
        ...
        if not probs: print("  Problems : none clearly detected")
        else:
            print("  Problems :")
            for p in probs[:4]:
                ...
                if p.get('tip_ja'): print(f"             -> {p['tip_ja']}")
        print(f"  Coach JA : {result.get('summary_ja','')}")
        print(f"  Used     : {used}")
        print(f"  Total    : {total:.2f}s\n")
    print("-"*86)
    print(f"Success        : {ok_count}/{len(rows)}")
    if total_times: print(f"Average latency: {sum(total_times)/len(total_times):.2f}s")
    print("Model wins     :")
    for m,c in sorted(wins.items(), key=lambda x:(-x[1],x[0])): print(f"  {m}: {c}")
